...
durri <- durr %>% select(uID, ASD..speakingtime.nsyll., articulation.rate..nsyll...phonationtime., speechrate..nsyll.dur., phonationtime..s., dur..s., npause, nsyll)
pitchi <- pitch %>% select(uID, diagnosis, trial, study, mean, sd, min, max, median, IQR, MAD, CV)
...
bydiag <- comp %>% group_by(diagnosis) %>% dplyr::summarise(n = n(), FemaleN = sum(Gender == "F"), Meanage = mean(Age), MeanSANS = mean(SANS), MeanSAPS = mean(SAPS))
# The same, but by both study and diagnosis
bystudy <- comp %>% group_by(diagnosis, study) %>% dplyr::summarise(n = n(), FemaleN = sum(Gender == "F"), Meanage = mean(Age), MeanSANS = mean(SANS), MeanSAPS = mean(SAPS))
